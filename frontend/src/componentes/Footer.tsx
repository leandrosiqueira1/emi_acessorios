"use client"; // necessário para hooks e styled-jsx
...
const Footer = () => {
  ...
  useEffect(() => {
    // Só calcula o ano no cliente → evita hydration mismatch
  ...
  return (
    <footer className="bg-black text-gray-200 mt-10 relative overflow-hidden">
          ...
          <ul className="space-y-2 text-sm">
            {["Início", "Produtos", "Contato", "Sobre nós"].map((item) => (
              <li key={item}>
                <Link
                  href="#"
                  className="relative hover:text-white transition duration-300 link-glow"
                >
                  {item}
                </Link>
              </li>
      ...
      <div className="border-t border-gray-700 text-center py-4 text-sm text-gray-400 relative z-10">
        © {year ?? "..."} Emi Acessórios - Todos os direitos reservados
      </div>
